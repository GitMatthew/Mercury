...
public interface AdminUtility {
	...
	public static String GET_ALL_EVENTS="SELECT id_evento,eventi.id_status,status.nome_status,nome_evento,descrizione,data_inizio,data_fine FROM eventi inner join status on eventi.id_status=status.id_status WHERE status.id_status=2 || status.id_status=3 order by eventi.id_status";
